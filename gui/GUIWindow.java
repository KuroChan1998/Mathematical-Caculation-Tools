 ...
 * @Package gui 
 * @Description:  实现图形界面的类 
 * @author: JinZhiyun    
 * @date:   2019年3月14日 下午3:03:12   
 * @version V1.0 
...
public class GUIWindow {
	/**
	 * @Fields FrameStartX,FrameStartY,FrameWidth,FrameHeight: Frame的位置大小参数
	 */
	...
	/**
	 * @Title: GUIWindow
	 * @Description: 构造函数初始化 Frame的位置大小参数
	 * @param: @throws
	 */
	public GUIWindow() {
		FrameStartX = 100;
	...
	public void setFrame() {
		...
		WindowOfCaculMod.GUICaculMod(frm); // 设置最小非负余数和绝对值最小余数计算模块

		WindowOfCommonFactorMultiple.GUICommonFactorMultiple(frm); // 设置最大公因数和最小公倍数的计算模块

		WindowOfEulerFuction.GUIEulerFuction(frm); // 设置欧拉函数值的计算模块
		...
		WindowOfGoldwasserMicaliBinaryEncryption.GUIGoldwasserMicaliBinaryEncryption(frm); // 设置GoldwasserMicali加密算法（对二进制加密）
																							// 模块
		WindowOfZgsydl.GUIZgsydl(frm); // 设置中国剩余定理计算模块

		WindowOfPolynomial.GUIPolynomial(frm); // 设置多项式计算模块

		WindowOfEllipticCurveCaculationOfFp.GUIEllipticCurveCaculationOfFp(frm); // 设置Fp上的椭圆曲线上的点的计算模块

		WindowOfEllipticCurveCaculationOfF2n.GUIEllipticCurveCaculationOfF2n(frm); // F2n上的椭圆曲线y^2+xy=x^3+a2*x^2+a6的点的计算
